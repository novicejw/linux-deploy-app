...
# JSON APIs to view categories, items and users
...
def showCategories():
    ...
    items = session.query(Item).order_by(desc(Item.time)).limit(10)
    if 'username' not in login_session:
...
def editCategory(category_id):
    ...
    if editedCategory.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not authorized to edit the name of this category.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
...
def showItems(category_id):
    ...
    num = len(items)
    if 'username' not in login_session:
        return render_template('publicitems.html', items=items, category=category, categories=categories, creator=creator, num=num)
    else:
        return render_template('items.html', items=items, category=category, categories=categories, creator=creator, num=num)
...
def newItem(category_id):
    if request.method == 'POST':
        ...
        return redirect(url_for('showItem', category_id=category_id, item_id=newItem.id, item=newItem))
    else:
...
def editItem(category_id, item_id):
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('showItem', category_id=category_id, item_id=item_id, item=editedItem))
    else:
